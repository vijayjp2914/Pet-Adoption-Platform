...
const SubmitPetPage = ({ onShowLogin }) => {
    ...
    return (
        <section className="py-12 px-4 max-w-4xl mx-auto mt-10 bg-white rounded-2xl shadow-xl animate-fade-in">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-lg mx-auto">
                <select id="type" value={petData.type} onChange={handleChange} required className="p-3 border-2 border-gray-200 rounded-lg focus:border-indigo-500">
                    <option value="dog">Dog</option>
                    <option value="cat">Cat</option>
                    <option value="bird">Bird</option>
                </select>
                <input type="text" id="name" placeholder="Pet Name" value={petData.name} onChange={handleChange} required className="p-3 border-2 border-gray-200 rounded-lg focus:border-indigo-500" />
                <input type="text" id="breed" placeholder="Breed" value={petData.breed} onChange={handleChange} required className="p-3 border-2 border-gray-200 rounded-lg focus:border-indigo-500" />
                <select id="age" value={petData.age} onChange={handleChange} required className="p-3 border-2 border-gray-200 rounded-lg focus:border-indigo-500">
                    <option value="puppy">Puppy</option>
                    <option value="adult">Adult</option>
                    <option value="senior">Senior</option>
                </select>
                <input type="text" id="health" placeholder="Health Condition" value={petData.health} onChange={handleChange} required className="p-3 border-2 border-gray-200 rounded-lg" />
                <input type="text" id="behavior" placeholder="Behavior" value={petData.behavior} onChange={handleChange} required className="p-3 border-2 border-gray-200 rounded-lg" />
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        {applications.map(app => (
                             <tr key={app._id} className="border-b hover:bg-gray-50">
                                ...
                                <td className="p-3">{app.petId?.name || 'N/A'}</td>
                                ...
                                <td className="p-3">
                                    {app.status === 'Pending' ? (
                                        ...
                                        <span className={`font-semibold ${app.status === 'Approved' ? 'text-green-600' : 'text-gray-500'}`}>{app.status}</span>
                                    )}
                                </td>
                            </tr>
                        ))}
                     ...
                     {applications.length === 0 && <p className="text-center text-gray-500 mt-4">No applications received yet.</p>}
                </div>
            </div>
        </section>
    );
